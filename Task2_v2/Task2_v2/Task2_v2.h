...
int main()
{
	...
	operation o1[100];
	folder MyRoot = folder("root", "00:00", "00:00", root);
	fs[0] = MyRoot;
			...
			if (y == 1) {
				...
				for (int i = 0; i < 1000; i++) {
					if (fs[i].getName() == name) {
						if (fs[i].getFolderType() != root) {
							fs[i] = fs[i].deleteSmht();
							break;
						}
						else
						{
			...
			if (y == 1) {
				...
				for (int i = 0; i < 1000; i++) {
					if (ls[i].getName() == name) {
						if (ls[i].getEnum() == tough) {
							for (int i = 0; i < 1000; i++) {
								if (fs[i].getName() == ls[i].getlink().getName()) {
									if (fs[i].getFolderType() != root) {
										fs[i] = fs[i].deleteSmht();
